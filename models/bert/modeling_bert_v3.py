...
class MyBertEmbeddings(nn.Module):
    ...
    def forward(
        self, input_ids=None, lattice=None, seq_length=None, sub_length=None
    ):
        ...
        if self.position_embedding_type == "absolute":
            ...
            #for i in range(sub_length.shape[0]):
            #    for j in lattice[i][sub_length[i]:]:
            #        if j == -100:
            #            break
            #        position_embeddings[:, i, sub_length[i]+j] = position_embeddings[:, i, j]
...
class BertForMaskedLM_v2(BertPreTrainedModel):
    ...
    def forward(
        ...
        #    print("logits: ", prediction_scores.shape)
...
class BetterBertModelForCSC(BertPreTrainedModel):
    ...
    def forward(
        ...
        sequence_output = outputs[0]
        #prediction_scores = self.cls(sequence_output)
        prediction_scores = torch.matmul(sequence_output, self.bert.embeddings.word_embeddings.weight.T)
        #print(input_ids)
        masked_lm_loss = None
        if labels is not None:
            loss_fct = CrossEntropyLoss()  # -100 index = padding token
...
@add_start_docstrings(""" Bert Model with a `language modeling` head on top. """, BERT_START_DOCSTRING)
class GectorModel(BertPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = 3805 + 1 # ./scripts/sighan_v1/vocabs.txt + 1 (token 0)
