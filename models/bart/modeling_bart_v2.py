...
""" PyTorch BART model. modified from transformers==4.4.1"""
import copy
import math
import random
import warnings
...
class BartModel(BartPretrainedModel):
    def __init__(self, config: BartConfig):
        ...
        encoder_config = BertConfig(
            ...
            attention_probs_dropout_prob=config.activation_dropout,
        )
...
@add_start_docstrings(
    "The bare BART Encoder Model outputting raw hidden-states without any specific head on top.",
    BART_START_DOCSTRING,
)
class BartForMaskedLM(BartPretrainedModel):
    """
    Only Use Bart Encoder for BartMaskedLM.
    """
    def __init__(self, config: BartConfig):
        super().__init__(config)
        encoder_config = BertConfig(
...
class CPTForSequenceClassification(BartPretrainedModel):
    def __init__(self, config: BartConfig, cls_mode=3, **kwargs):
...
if __name__ == '__main__':
    # model_path = '/remote-home/source/yfshao/bart-zh/arch12-2-new-iter8w/encoder'
    model_path = '/remote-home/share/yfshao/bart-zh/arch12-2-new-iter10w/'
    # model_path = '/remote-home/source/yfshao/bart-zh/arch24-4-new-iter1w/encoder'
    from transformers import BertTokenizer, BartConfig, BertConfig
    contexts = ["所以我迟到了。今天下雨， ", "学习我不想每天"]
    # config = BartConfig.from_pretrained(model_path)
    encoder_config = BertConfig.from_pretrained('hfl/chinese-roberta-wwm-ext')
    tokenizer = BertTokenizer.from_pretrained('hfl/chinese-roberta-wwm-ext')
    model = BartForConditionalGeneration.from_pretrained(model_path)
    # model = BertModel.from_pretrained(model_path, config=encoder_config)
    model.eval()
    # model = BartModel.from_pretrained()
    # model.save_pretrained('bart-v2')
    # state_dict = torch.load('bart-v2/pytorch_model.bin')
    # print(state_dict.keys())

    inputs = tokenizer.batch_encode_plus(contexts, padding=True, max_length=512, return_token_type_ids=False)
    #input_ids = torch.LongTensor(inputs['input_ids'])
    #attn_mask = torch.LongTensor(inputs['attention_mask'])
    ...
    #pred_ids = model.generate(input_ids, max_length=20)
    ...
    print(inputs)
    train_dataset = mydataset(features)
