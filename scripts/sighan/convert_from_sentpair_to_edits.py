...
with open(src_path) as f_src, open(tgt_path) as f_tgt, open(sid_path) as f_sid:
    ...
    #print(len(lines_src), len(lines_tgt), len(lines_sid))
    assert len(lines_src) == len(lines_tgt) == len(lines_sid)

    for i in range(len(lines_src)):
        #print("raw", lines_src[i])
        src_line = unicodedata.normalize('NFKC', lines_src[i].strip().replace(' ', '').replace('“', '\"').replace('”', '\"'))
        #print("src", src_line)
        #tgt_line = lines_tgt[i].strip().replace(' ', '').replace(',', '，').replace('(', '（').replace(')', '）').replace(':', '：')
        tgt_line = unicodedata.normalize('NFKC', lines_tgt[i].strip().replace(' ', ''))
        #print("tgt", tgt_line)
        sid = lines_sid[i].strip().split('\t')[0]
        edits = Levenshtein.opcodes(src_line, tgt_line)
        result = []
        for edit in edits:
            if ("。" or "“" or "”") in tgt_line[edit[3]:edit[4]]: # rm 。
                continue
            if edit[0] == "insert":
